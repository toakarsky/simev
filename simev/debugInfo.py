...
class HoveredObjectsList:
    ...
    def getHoveredObjectInfo(self, renderScreen):
        if self.hoveredObjects:
            d = DebugInfo(
                renderScreen, self.hoveredObjects[-1].getDebugInfo(renderScreen), DEBUG_INFO_POS_ENUM.BOTTOM_RIGHT)
...
class DebugInfo:
    def __init__(self, renderScreen, debugInfoTextStrs, position):
        ...
        for debugInfoTextStr in debugInfoTextStrs:
            debugInfoTexts.append(debugFont.render(
                debugInfoTextStr, True, (255, 255, 255)))
        ...
        for i in range(len(debugInfoTextStrs)):
            debugSurface.blit(
                debugInfoTexts[i], (20, (i * DEBUG_INFO_FONT_SIZE) + 15))
